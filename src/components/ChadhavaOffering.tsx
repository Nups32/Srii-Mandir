    ...
    <section className="max-w-6xl mx-auto px-4 py-8">
      <div className="grid md:grid-cols-2 gap-8 items-start">
        <div className="relative rounded-xl overflow-hidden shadow">
          <img src={images[current]} alt="Chadhava" className="w-full h-65 object-cover" />
          <button onClick={prevSlide} className="absolute top-1/2 left-3 -translate-y-1/2 bg-white/80 p-1 rounded-full">
            <ChevronLeft size={20} />
          </button>
          <button onClick={nextSlide} className="absolute top-1/2 right-3 -translate-y-1/2 bg-white/80 p-1 rounded-full">
        ...
        <div className="flex flex-col justify-start">
          <h1 className="text-2xl font-semibold text-gray-900">{chadhavaData.name}</h1>
          <p className="text-sm text-gray-600 mt-3">{chadhavaData.about}</p>
          ...
          <p className="mt-4 text-sm text-gray-700">
            Till now <span className="font-semibold text-orange-600">1,50,000+ devotees</span> have participated.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {chadhavaData.offering.map((item) => (
            <div
              ...
              className="flex justify-between gap-4 p-4 cursor-pointer border-b border-gray-200"
            >
              ...
              <div className="flex flex-col justify-center items-center gap-2">
                ...
                {cart[item.id] ? (
                  <div className="flex items-center gap-2 bg-green-600 text-white rounded-md px-2 py-1" onClick={(e) => e.stopPropagation()}>
      ...
      {totalCount > 0 && (
        <div
          onClick={() => navigate("/chadhava-cart")}
