...
import { getShaktiSanyansiForEdit, updateShaktiSanyansi } from "@/utils/API";
...
const EditShaktiSanyansi: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setFetching(true);
            try {
                const response = await getShaktiSanyansiForEdit(id);
                if (response.data.status && response.data.data) {
                    ...
                    setTeachingItems(data.teaching || []);
                    setPracticeItems(data.practice || [{ title: "", description: [""] }]);
                    ...
                    if (data.image) {
                        ...
                        setFileList([{
                            ...
                            name: data.image,
                            status: 'done',
                            url: `${import.meta.env.VITE_APP_Image_URL}/shakti-sanyansi/${data.image}`,
                        }]);
                    }
                } else {
                ...
                message.error("Failed to load Shakti Sanyansi");
                navigate("/admin/shakti-sanyansi");
            } finally {
                setFetching(false);
            }
    ...
    const handleRemove = () => {
        setFileList([]);
        // Don't clear existingImage here, we need to know if we should delete it
    };
    ...
    const onFinish = async (values: any) => {
        ...
        // If new image is uploaded, append it
        if (fileList[0]?.originFileObj) {
            formData.append('image', fileList[0].originFileObj);
        }
        // If image was removed, we need to handle this on backend
        // Backend will check if req.file exists and handle accordingly

        try {
            const response = await updateShaktiSanyansi(id, formData);
            if (response.data.status) {
                message.success('Shakti Sanyansi updated successfully');
                navigate('/admin/shakti-sanyansi');
            } else {
                message.error(response.data.message || 'Failed to update Shakti Sanyansi');
            }
        } catch (error: any) {
            console.error('Error updating Shakti Sanyansi:', error);
            message.error(error.response?.data?.message || 'Failed to update Shakti Sanyansi');
        } finally {
            setLoading(false);
        }
    };
    ...
    if (fetching) {
        return (
            <div className="flex justify-center items-center h-64">
                <Spin size="large" />
                <span className="ml-4">Loading Shakti Sanyansi...</span>
            </div>
        );
    }
        ...
        <div className="p-4">
            <Row className="mb-4" align="middle">
                <Col xs={17} sm={17} md={20} xl={20} xxl={20}>
                    <h2 className="text-2xl font-bold">Edit Shakti Sanyansi</h2>
                </Col>
            ...
            <Form form={form} layout="vertical" onFinish={onFinish}>
                ...
                <Card className="mb-4">
                    ...
                    <div className="mb-4">
                        ...
                        {existingImage && fileList.length === 0 && (
                            <div className="mt-4">
                                <div className="mb-2">Current Image:</div>
                                <Image
                                    src={`${import.meta.env.VITE_APP_Image_URL}/shakti-sanyansi/${existingImage}`}
                                    alt="Current"
                                    width={200}
                ...
                <div className="flex justify-start gap-4 m-10">
                    ...
                    <Button
                        ...
                        size="large"
                    >
                        {loading ? <Spin size="small" /> : 'Update Shakti Sanyansi'}
                    </Button>
                </div>
            </Form>
        </div>
...
export default EditShaktiSanyansi;