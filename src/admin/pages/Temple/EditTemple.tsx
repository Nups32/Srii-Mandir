...
const EditTempleForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTempleData = async () => {
      if (!id) {
        message.error("No temple ID provided");
      ...
      try {
        ...
        if (res.data.status && res.data.data) {
          ...
          message.error(res.data.message || "Failed to fetch temple data");
          navigate("/admin/temple");
        }
      } catch (error: any) {
        console.error("Error fetching temple:", error);
        message.error(error.response?.data?.message || "Failed to load temple data");
        navigate("/admin/temple");
      } finally {
        setFetching(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    if (!id) {
      message.error("No temple ID found");
      return;
    }
    ...
    try {
      ...
      message.error(error.response?.data?.message || "Failed to update temple. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="flex justify-center items-center h-64">
        <Spin size="large" />
        <span className="ml-4">Loading temple data...</span>
      </div>
    );
  }
    ...
    <div className="">
      <Row className="m-2" align="middle">
        <Col xs={17} sm={17} md={20} xl={20} xxl={20}>
          <h2 className="text-2xl font-bold">Edit Temple</h2>
        </Col>
              ...
              >
                {loading ? (
                  <div className="flex items-center">
                    <Spin size="small" />
                    <span className="ml-2!">Updating Temple...</span>
                  </div>
                ) : (
                  "Update Temple"
                )}
