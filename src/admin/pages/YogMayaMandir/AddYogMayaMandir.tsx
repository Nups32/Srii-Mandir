...
import { createYogMayaMandir } from "@/utils/API";
...
const AddYogMayaMandirForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await createYogMayaMandir(formData);
      if (res.data.status) {
        message.success("Yog Maya Mandir content created successfully");
      ...
      message.error(error.response?.data?.message || "Failed to create content. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="">
      <Row className="m-2" align="middle">
        <Col xs={17} sm={17} md={20} xl={20} xxl={20}>
          <h2 className="text-2xl font-bold">Add New Yog Maya Mandir Content</h2>
        </Col>
...
export default AddYogMayaMandirForm;