...
import { storePoojaPackage } from "@/utils/API";
import TextArea from "antd/es/input/TextArea";

const AddPoojaPackageForm: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await storePoojaPackage(formData);
            if(res.data.status){
                message.success("Pooja package added successfully");
                navigate("/admin/pooja-package");
            }else{
                message.error('server error');
            }
        } catch (error) {
            console.error("Error adding pooja package:", error);
            message.error("Failed to add pooja package. Please try again.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="">
            <Row className="m-2" align="middle">
                <Col xs={17} sm={17} md={20} xl={20} xxl={20}>
                    <h2 className="text-2xl font-bold">Add Pooja Package</h2>
                </Col>
                            ...
                            >
                                {loading ? (
                                    <div className="flex items-center">
                                        <Spin size="small" />
                                        <span className="!ml-2">Adding Package...</span>
                                    </div>
                                ) : (
                                    "Add Package"
                                )}
...
export default AddPoojaPackageForm;